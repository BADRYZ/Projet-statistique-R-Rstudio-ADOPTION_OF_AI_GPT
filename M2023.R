...
library(readxl)
M <- read_excel("C:/Users/Zakaria/Desktop/transcrip/form.xlsx")
View(M)
...
#### itemA3##
boxplot(M$itemA3)### pas de valeur abberantes
#### itemA4 ###"
boxplot(M$itemA4)
...
####  itemUT2 ####
boxplot(M$itemUT2) #pas de valeur abberantes 
####  itemUT3 #####
boxplot(M$itemUT3) #pas de valeur abberantes 
####  itemUT4 #####
boxplot(M$itemUT4)
...
#si c est pas possible on supprime ou on estime (si la proportion des valeur manqu est importante(>5%) on va estime si non  on supprime (si apres suppresion on reste superieur a 30) si non estime )
#dans notre cas estime 
#nbr valeur manqunte
...
for (i in (1:length(M$age)))
{
  ...
  if (M$age[i] %in% boxplot.stats(M$age)$out)
    M$age[i]=as.integer(mean(M$age,na.rm = TRUE))#sans prendre en consideration les na 
}
...
#####A revoirrrrrr le cas de suprresion 
for (i in 1:length(M$itemA1)) {
  if (is.na(M$itemA1[i])) {
...
#3.3 : Test de significativite des variables de mesure

#3.3.1 : tests de normalite
...
#les deux proportionnalités sont significativement différents, 
...
#Maintenant on passe au test d ANOVA
...
#p-value=0.8339 , il y a une correlation
